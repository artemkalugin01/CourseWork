...
#Чтение данных
Data = pd.read_csv('/Users/artemkalugin/Desktop/Partial_Dataset_1.csv', sep=';')
blacklist = pd.read_csv('/Users/artemkalugin/Desktop/March_blacklist_1.csv',sep=';')
blacklist = blacklist.drop(['Unnamed: 0'],axis=1)
frames=[Data,blacklist]
X=pd.concat(frames)
columns = X.columns
...
res_mapping = {'blacklist':1,'background':0}
X['res'] = X['res'].map(res_mapping)
y = X['res']
#X=X.drop(['res'], axis = 1)
pr_mapping = {'TCP':1.,'UDP':2.,'ICMP':3.,'ESP':4.,'GRE':5.,'IPIP':6.}
X['pr'] = X['pr'].map(pr_mapping)

flg_mapping = {'.A....':1.,'.AP.SF':2.,'.AP.S.':3.,'.AP...':4.,'.APRSF':5.,'.A...F':6.,'.APRS.':7.,'....S.':8.,'.AP..F':9.,'.A..SF':10.,'.A.R..':11.,'.APR..':12.,'.A..S.':13.,
               '...R..':14.,'.A.R.F':15.,'.APR.F':16.,'.A.RS.':17.,'...RS.':18.,'.A.RSF':19.,'......':20}
X['flg'] = X['flg'].map(flg_mapping)
flaglist = X['flg']

X['te'] = time_to_minutes(X['te'])
X['td'] = td_to_float(X['td'])

#изменение IP адресов
X['sa'] = coher_of_values(X['sa'])
X['da'] = coher_of_values(X['da'])



X.to_csv('/Users/artemkalugin/Desktop/Fixed_Partial_Dataset_1.csv')
"""
...
optimizer = Optimizer_RMSprop(learning_rate=0.01, decay=1e-5, rho=0.999)
#optimizer = Optimizer_Adam(learning_rate=0.1, decay=1e-8)
...
for epoch in range(120):
    ...
    if not epoch % 5:
        print('epoch:', epoch, 'acc:', f'{accuracy:.3f}', 'loss:', f'{loss:.4f}', '(data_loss:', f'{data_loss:.4f}',
              'reg_loss:', f'{regularization_loss:.4f})', ')', 'lr:', f'{optimizer.current_learning_rate:.5f}')
        sys.stdout.flush()
...
print(f'validation, acc: {accuracy:.3f}, loss: {loss:.5f}')
sys.stdout.flush()

# Saving weights and biases to JSON file
dict_dence1 = {'weights':dense1.weights.tolist(),'biases':dense1.biases.tolist()}
dict_dence2 = {'weights':dense2.weights.tolist(),'biases':dense2.biases.tolist()}

dences = []
dences.append(dict_dence1)
dences.append(dict_dence2)

filename = 'dences.txt'
myfile = open(filename,mode='w+')
json.dump(dences,myfile)
myfile.close()